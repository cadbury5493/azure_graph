...
def process_json_object(s3, bucket, key):
    ...
    # Collect results but keep them grouped
    results = []
    profiles = data.get("profiles", [])
    for profile in profiles:
        for control in profile.get("controls", []):
    ...
    overall_status = "failed" if any(r["status"].lower() == "failed" for r in results) else "passed"
...
def main():
    ...
    for key in get_s3_objects(bucket, prefix, s3):
        print(f"Processing: {key}")
        record = process_json_object(s3, bucket, key)
